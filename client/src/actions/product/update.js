...
export function retrieveError(retrieveError) {
  return { type: 'PRODUCT_UPDATE_RETRIEVE_ERROR', retrieveError };
}

export function retrieveLoading(retrieveLoading) {
  return { type: 'PRODUCT_UPDATE_RETRIEVE_LOADING', retrieveLoading };
}

export function retrieveSuccess(retrieved) {
  return { type: 'PRODUCT_UPDATE_RETRIEVE_SUCCESS', retrieved };
}
...
export function updateSuccess(updated) {
  return { type: 'PRODUCT_UPDATE_UPDATE_SUCCESS', updated };
}
...
export function mercureOpen(eventSource) {
  return { type: 'PRODUCT_UPDATE_MERCURE_OPEN', eventSource };
}

export function mercureMessage(retrieved) {
  return dispatch => {
    if (1 === Object.keys(retrieved).length) {
      dispatch({ type: 'PRODUCT_UPDATE_MERCURE_DELETED', retrieved });
      return;
    }

    dispatch({ type: 'PRODUCT_UPDATE_MERCURE_MESSAGE', retrieved });
  };
}
