            ...
            if ($class['isEnum']) {
                ...
                foreach ($this->graphs as $graph) {
                    foreach ($graph->allOfType($type->getUri()) as $instance) {
                        ...
                        throw new \Exception($instance->localName());
                        throw new \Exception($class['constants'][$instance->localName()]['name']);
                    }
                }
            } else {
                // Entities
    ...
    private function generateField(array $config, array $class, \EasyRdf_Resource $type, $typeName, $propertyName, \EasyRdf_Resource $property = null)
    {
        ...
        if (0 === $numberOfRanges) {
            ...
            $class['fields'][$propertyName] = [
                ...
                'resource' => $property,
                'range' => $propertyConfig['range'],
                'cardinality' => $cardinality,
                'ormColumn' => $propertyConfig['ormColumn'] ?? null,
                'typeHint' => $propertyConfig['typeHint'] ? true : false,
                'isArray' => $isArray,
